...
import { Category, Group_Projects, Rule } from "@/lib/supabase/supabase.types";
...
const Page = () => {
  ...
  const [environemntId, setEnvironemntId] = useState("");
  ...
  useEffect(() => {
    const fetchallSkillsData = async () => {
      ...
      // setEnvironemntId(environemnt.data.id);
    };
  ...
  const fetchData = async () => {
    setDataLoading(true);
    const rulesData: RuleWithAllSkill | null = await fetchRules(environemntId);
    console.log(rulesData);
    // console.log("rules:", rulesData);
    // const apexians = await fetchEnvProfiles();
    const environemnt = await fetchEnvironmentDetails(environemntId);
    console.log(environemnt);
    ...
    let updatedEnvironment: EnvironmentWithProfilesWithRuleWithCategoryWithGroupprojects;
    updatedEnvironment = environemnt ? environemnt : { id: environemntId };
    ...
    Environmentform.reset({
      name: environemnt?.name ? environemnt?.name : "",
      description: environemnt?.description ? environemnt?.description : "",
      environment_imgUrl: environemnt?.environment_imgUrl
        ? environemnt?.environment_imgUrl
        : "",
      access_type: environemnt?.access_type
        ? environemnt?.access_type
        : "PUBLIC",
      rule_id: environemnt?.rule_id ? environemnt?.rule_id : "",
    });
  ...
  > = async (formData) => {
    ...
    try {
      ...
      Environmentform.reset();
      router.push(`/create-environment/?env=${environemnt.data.id}`);
    } catch (error) {
      console.log("Error at axios: ", error);
    }
  };
                ...
                >
                  ...
                  <div className="flex items-center justify-center">
                    <Button
                      type="submit"
                      variant="ghost"
                      className="border-2 border-black bg-black text-white hover:bg-gray-300 w-[93%]"
                    >
                      Save changes
                    </Button>
                  </div>
