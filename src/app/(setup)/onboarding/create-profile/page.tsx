...
import { CalendarIcon, Edit, Pen, Plus } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogFooter,
...
import { Qualifications, profile } from "@/lib/supabase/supabase.types";
import { useToast } from "@/components/ui/use-toast";
import { fetchQualifications } from "@/lib/supabase/queries";
...
const Page = () => {
  ...
  const [qualifications, setQualifications] = useState<Qualifications[] | null>(
    []
  );
  ...
  const onSubmit: SubmitHandler<z.infer<typeof ProfileFormSchema>> = async (
    ...
    try {
      ...
      dispatch({
        type: "UPDATE_PROFILE",
        payload: { profile: profile.data as profile },
      });
      toast({
  ...
  > = async (formData) => {
    ...
    try {
      ...
      dispatch({
        ...
        payload: { qualification: qualification.data as Qualifications },
      });
      toast({
        title: "Qualification Added",
        description: `${qualification.data.qualification} has been added successfully.`,
      });
            ...
            <Dialog key={index}>
              ...
              >
                <div className="flex   w-full items-center">
                  ...
                  <div className="font-semibold text-lg flex border-2 p-2 rounded-[4px] cursor-pointer">
                    <DialogTrigger
                      asChild
                      className="bg-white text-black hover:bg-white"
                    >
                ...
                <div className="flex   w-full  mx-4  items-center">
                  <div className="  text-md px-1">
                    ...
                    {qualification.field_of_study} - {qualification.grade} CGPA
                  </div>
                </div>
                <div className="flex   w-full  mx-4  items-center">
                  <div className="  text-md px-1">
                    {qualification.start_date.toISOString().substring(0, 10)} -{" "}
                    {qualification.end_date.toISOString().substring(0, 10) ===
                    qualification.createdAt.toISOString().substring(0, 10)
                      ? "Present"
                      : qualification.end_date.toISOString().substring(0, 10)}
                  </div>
                </div>
